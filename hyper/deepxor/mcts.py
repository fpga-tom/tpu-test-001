...
import numpy as np
from deepxor.deepxor import num_actions
...
class MCTSNode(object):
    ...
    def __init__(self, position, fmove=None, parent=None):
        ...
        self.original_prior = np.zeros([num_actions], dtype=np.float32)
        self.child_prior = np.zeros([num_actions], dtype=np.float32)
        self.children = {}
    ...
    def incorporate_results(self, move_probabilities, value, up_to):
        ...
        self.child_W = np.ones([num_actions], dtype=np.float32) * value
    ...
    def inject_noise(self):
        ...
        a = legal_moves * ([FLAGS.dirichlet_noise_alpha] * (num_actions))
